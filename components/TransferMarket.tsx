...
export const TransferMarket: React.FC<TransferMarketProps> = ({ teams, currentUser, onViewPlayer }) => {
    ...
    const filteredAgents = freeAgents.filter(u => 
        u.name.toLowerCase().includes(filter.toLowerCase()) || 
        u.position?.toLowerCase().includes(filter.toLowerCase())
    );
    ...
    const handleInvitePlayer = async (player: User) => {
        // Mock invitation logic for now (notification system handles requests from players, 
        // invitiations from owners could be implemented similarly)
        if (!currentUser.teamId) {
            alert("Você precisa ter um time criado para contratar.");
            return;
        }
        
        // Simulating invite sent
        setRequestedItems(prev => new Set(prev).add(player.id));
        alert(`Convite enviado para ${player.name}! (Simulação)`);
        // In real impl: await dbService.sendTeamInvite(currentUser.teamId, player.id);
    };
    ...
    return (
        <div className="space-y-6 pb-24">
            ...
            <div className={`p-6 rounded-3xl border border-white/10 relative overflow-hidden shadow-2xl ${isOwner ? 'bg-gradient-to-r from-gold/20 to-pitch-900' : 'bg-gradient-to-r from-blue-900 to-pitch-900'}`}>
                <div className="absolute top-0 right-0 text-9xl opacity-10 rotate-12 select-none">
                    {isOwner ? '📋' : '⚽'}
                </div>
                ...
                <p className="text-gray-300 text-sm relative z-10 max-w-xs mt-2">
                    {isOwner 
                        ? 'Encontre reforços para o elenco. Analise stats e envie propostas.' 
                ...
                <div className="mt-6 relative z-10">
                    <input 
                        type="text" 
                        placeholder={isOwner ? "Buscar jogador por nome ou posição..." : "Buscar por time ou bairro..."}
                        value={filter}
            ...
            {isOwner ? (
                <div className="space-y-4">
                    ...
                    {isLoadingAgents ? (
                        ...
                        <div className="text-center py-12 bg-white/5 rounded-2xl border border-dashed border-white/10">
                            <p className="text-gray-500 font-bold">Nenhum jogador livre encontrado.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {filteredAgents.map((agent, idx) => (
                                <div 
                                    key={agent.id}
                                    onClick={() => onViewPlayer && onViewPlayer(agent)}
                                    className="bg-white/5 border border-white/5 rounded-xl p-3 flex items-center gap-4 hover:border-gold/30 hover:bg-white/10 transition-all cursor-pointer group"
                                    style={{ animationDelay: `${idx * 0.05}s` }}
                                >
                                    <div className="relative">
                                        <img src={agent.avatarUrl} className="w-14 h-14 rounded-full object-cover bg-gray-800 border border-white/10" />
                                        {/* Position Badge */}
                                        <div className="absolute -bottom-1 -right-1 bg-black text-gold text-[9px] font-bold px-1.5 py-0.5 rounded border border-gold/30">
                                            {agent.position || '?'}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex gap-2 mt-1">
                                            {agent.stats?.rating ? (
                                                <span className="text-[10px] bg-white/10 text-white px-1.5 rounded font-mono">OVR {agent.stats.rating}</span>
                                            ) : (
                                                <span className="text-[10px] text-gray-500">Sem Rank</span>
                                            )}
                                    ...
                                    <button 
                                        onClick={(e) => { e.stopPropagation(); handleInvitePlayer(agent); }}
                                        disabled={requestedItems.has(agent.id)}
                                        className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${
                                            requestedItems.has(agent.id) 
                ...
                /* --- PLAYER/FAN VIEW: HIRING CLUBS --- */
                <div className="space-y-4">
                    <h3 className="text-white font-bold uppercase tracking-widest text-xs ml-2 mb-2 flex items-center gap-2">
                        <span className="w-2 h-2 bg-neon rounded-full animate-pulse"></span>
                        Clubes com Seletivas ({filteredTeams.length})
                    </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredTeams.map((team, idx) => (
                            <div 
                                key={team.id} 
                                className="bg-white/5 border border-white/5 rounded-2xl p-4 flex items-center gap-4 hover:border-neon/30 hover:bg-white/10 transition-all group shadow-lg backdrop-blur-md animate-[fadeIn_0.3s_ease-out]"
                                style={{ animationDelay: `${idx * 0.05}s` }}
                            >
                                ...
                                <div className="flex flex-col gap-2">
                                     {currentUser.teamId === team.id ? (
                                         ...
                                         <button 
                                            ...
                                            disabled={requestedItems.has(team.id)}
            ...
            {/* TEAM ROSTER MODAL (For Players/Fans) */}
            {viewingTeamRoster && (
