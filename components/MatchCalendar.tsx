...
export const MatchCalendar: React.FC<MatchCalendarProps> = ({ matches, teams, currentUser }) => {
  ...
  // 1. Filter matches relevant to the user (My Team or Followed Teams)
  // If user has no team and follows no one, show all (Global View)
  const relevantMatches = matches.filter(match => {
      const myTeamId = currentUser.teamId;
      if (myTeamId) {
          return match.homeTeamId === myTeamId || match.awayTeamId === myTeamId;
      }
      // If fan, show followed matches, else show all
  ...
  const renderMatchCard = (match: Match) => {
      ...
      // Determine Result Status (only for past)
      let resultColor = 'border-gray-700 bg-gray-800/50';
      let resultLabel = '';
      
      if (activeTab === 'past' && currentUser.teamId) {
      ...
      return (
          <div key={match.id} className={`relative flex flex-col md:flex-row items-stretch bg-pitch-900 border rounded-2xl overflow-hidden mb-4 transition-all hover:scale-[1.01] ${activeTab === 'past' ? resultColor : 'border-white/10 hover:border-neon/30'}`}>
              
              {/* Date Strip */}
              <div className={`flex flex-row md:flex-col items-center justify-center p-4 min-w-[80px] ${activeTab === 'upcoming' ? 'bg-neon text-pitch-950' : 'bg-black/40 text-gray-400'} font-display font-bold`}>
                  ...
                  <div className="flex items-center justify-between">
                      ...
                      <div className="w-1/3 flex flex-col items-center justify-center">
                          {activeTab === 'past' ? (
                              ...
                              <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-500 font-display font-bold text-xl border border-white/10">
                                  VS
                              </div>
                          )}
                      </div>
        ...
        <div className="animate-[fadeIn_0.3s_ease-out]">
            {displayMatches.length === 0 ? (
                ...
                <div className="space-y-4">
                    {/* Month Separator Logic could go here for extra polish */}
